...
class mockModel_3D(object):
    ...
    def GetDataCacheFunctions(self):
        ...
        functionList.append(
            [pyeq3.DataCache.DataCacheFunctions.Y(NameOrValueFlag=1, args=[2.0]), [2.0]]
        )
        return functionList


service = pyeq3.Services.DataConverterService.DataConverterService()


class TestDataCache(unittest.TestCase):
    def test_ReducedDataSize_2D(self):
        model = mockModel_2D("SSQABS")
        service.ConvertAndSortColumnarASCII(
            DataForUnitTests.asciiIntegerDataInColumns, model, False
        )
    ...
    def test_DataCache_2D(self):
        ...
        model = mockModel_2D("SSQABS")
        service.ConvertAndSortColumnarASCII(
            DataForUnitTests.asciiDataInFourColumns_small, model, False
        )
        ...
        self.assertTrue(
            numpy.equal(
                model.dataCache.allDataCacheDictionary["X"], cached_X_ShouldBe
            ).all()
        )
        self.assertTrue(
            numpy.equal(
                model.dataCache.allDataCacheDictionary["ExpX"], cached_ExpX_ShouldBe
            ).all()
        )

        self.assertTrue(
            numpy.equal(
                model.dataCache.reducedDataCacheDictionary["X"], cached_X_ShouldBe
            ).all()
        )
        self.assertTrue(
            numpy.equal(
                model.dataCache.reducedDataCacheDictionary["ExpX"], cached_ExpX_ShouldBe
            ).all()
        )
    ...
    def test_DataCache_3D(self):
        ...
        self.assertTrue(
            numpy.equal(
                model.dataCache.allDataCacheDictionary["Y"], cached_Y_ShouldBe
            ).all()
        )
        ...
        self.assertTrue(
            numpy.equal(
                model.dataCache.reducedDataCacheDictionary["Y"], cached_Y_ShouldBe
            ).all()
        )
