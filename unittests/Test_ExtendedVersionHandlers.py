...
class TestExtendedVersionHandlers(unittest.TestCase):
    def test_ExtendedVersion_Exponential_WithOffset_2D(self):
        ...
        self.assertEqual(
            equation.extendedVersionHandler.__class__.__name__,
            "ExtendedVersionHandler_Offset",
        )
    ...
    def test_ExtendedVersion_Reciprocal_Exponential_2D(self):
        equation = pyeq3.Models_2D.Exponential.Exponential("SSQABS", "Reciprocal")
        self.assertEqual(
            equation.extendedVersionHandler.__class__.__name__,
    ...
    def test_ExtendedVersion_Reciprocal_Exponential_WithOffset_2D(self):
        equation = pyeq3.Models_2D.Exponential.Exponential(
        ...
        self.assertEqual(
            equation.GetDisplayHTML(), "y = a * exp(bx)<br>y = 1.0 / y + Offset"
        )
        self.assertEqual(
            equation.GetDisplayName(), "Reciprocal Exponential With Offset"
        )
        self.assertEqual(equation.GetCoefficientDesignators(), ["a", "b", "Offset"])
        self.assertEqual(len(equation.GetDataCacheFunctions()), 1)
        self.assertFalse(equation.CanLinearSolverBeUsedForSSQABS())
    ...
    def test_ExtendedVersion_Inverse_Exponential_WithOffset_2D(self):
        ...
        self.assertEqual(
            equation.extendedVersionHandler.__class__.__name__,
            "ExtendedVersionHandler_InverseWithOffset",
        )
        self.assertEqual(
            equation.GetDisplayHTML(), "y = a * exp(bx)<br>y = x / y + Offset"
        )
    ...
    def test_ExtendedVersion_Exponential_WithLinearDecay_2D(self):
        equation = pyeq3.Models_2D.Exponential.Exponential("SSQABS", "Linear Decay")
        self.assertEqual(
            equation.extendedVersionHandler.__class__.__name__,
            "ExtendedVersionHandler_LinearDecay",
        )
    ...
    def test_ExtendedVersion_Exponential_WithLinearDecayAndOffset_2D(self):
        equation = pyeq3.Models_2D.Exponential.Exponential(
            "SSQABS", "Linear Decay And Offset"
        )
        self.assertEqual(
            equation.extendedVersionHandler.__class__.__name__,
        ...
        self.assertEqual(
            equation.GetDisplayHTML(), "y = a * exp(bx)<br>y = y / x + Offset"
        )
        self.assertEqual(
            equation.GetDisplayName(), "Exponential With Linear Decay And Offset"
        )
        self.assertEqual(equation.GetCoefficientDesignators(), ["a", "b", "Offset"])
        self.assertEqual(len(equation.GetDataCacheFunctions()), 1)
        self.assertFalse(equation.CanLinearSolverBeUsedForSSQABS())
    ...
    def test_ExtendedVersion_Exponential_WithLinearGrowth_2D(self):
        equation = pyeq3.Models_2D.Exponential.Exponential("SSQABS", "Linear Growth")
        self.assertEqual(
            equation.extendedVersionHandler.__class__.__name__,
            "ExtendedVersionHandler_LinearGrowth",
        )
        self.assertEqual(equation.GetDisplayHTML(), "y = a * exp(bx)<br>y = y * x")
    ...
    def test_ExtendedVersion_Exponential_WithLinearGrowthAndOffset_2D(self):
        ...
        self.assertEqual(
            equation.GetDisplayName(), "Exponential With Linear Growth And Offset"
        )
        self.assertEqual(equation.GetCoefficientDesignators(), ["a", "b", "Offset"])
        self.assertEqual(len(equation.GetDataCacheFunctions()), 1)
        self.assertFalse(equation.CanLinearSolverBeUsedForSSQABS())
    ...
    def test_ExtendedVersion_Asymptotic_Exponential_A_WithExponentialGrowth_2D(self):
        equation = pyeq3.Models_2D.Exponential.AsymptoticExponentialA(
            "SSQABS", "Exponential Growth"
        )
        self.assertEqual(
            equation.extendedVersionHandler.__class__.__name__,
            "ExtendedVersionHandler_ExponentialGrowth",
        )
        self.assertEqual(
            equation.GetDisplayHTML(), "y = 1.0 - a<sup>x</sup><br>y = y * (b * exp(x))"
        )
        self.assertEqual(
            equation.GetDisplayName(),
            "Asymptotic Exponential A With Exponential Growth",
        )
        self.assertEqual(equation.GetCoefficientDesignators(), ["a", "b"])
    ...
    def test_ExtendedVersion_Asymptotic_Exponential_A_WithExponentialGrowthAndOffset_2D(
        self,
    ):
        equation = pyeq3.Models_2D.Exponential.AsymptoticExponentialA(
            "SSQABS", "Exponential Growth And Offset"
        )
        self.assertEqual(
            equation.extendedVersionHandler.__class__.__name__,
            "ExtendedVersionHandler_ExponentialGrowthAndOffset",
        )
        self.assertEqual(
            equation.GetDisplayHTML(),
            "y = 1.0 - a<sup>x</sup><br>y = y * (b * exp(x)) + Offset",
        )
        self.assertEqual(
            equation.GetDisplayName(),
            "Asymptotic Exponential A With Exponential Growth And Offset",
        )
        self.assertEqual(equation.GetCoefficientDesignators(), ["a", "b", "Offset"])
        self.assertEqual(len(equation.GetDataCacheFunctions()), 2)
        self.assertFalse(equation.CanLinearSolverBeUsedForSSQABS())
    ...
    def test_ExtendedVersion_Asymptotic_Exponential_A_WithExponentialDecay_2D(self):
        ...
        self.assertEqual(
            equation.GetDisplayName(), "Asymptotic Exponential A With Exponential Decay"
        )
        self.assertEqual(equation.GetCoefficientDesignators(), ["a", "b"])
        self.assertEqual(len(equation.GetDataCacheFunctions()), 2)
        self.assertFalse(equation.CanLinearSolverBeUsedForSSQABS())
